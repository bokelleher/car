<?php
// ✅ Church Taxonomy Meta Management (Fixed)
...
function car_edit_church_meta_fields($term) {
    ...
    $church = $wpdb->get_row(
        $wpdb->prepare("SELECT * FROM {$wpdb->prefix}car_churches WHERE term_id = %d LIMIT 1", $term_id)
    );
    ...
    $address = esc_textarea($church->address ?? '');
    ...
    <tr class="form-field">
        ...
        <td><input name="website" id="website" type="url" value="<?php echo $website; ?>" class="regular-text" /></td>
    </tr>
    <tr class="form-field">
...
function car_save_church_meta_fields($term_id) {
    ...
    $existing = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table WHERE term_id = %d", $term_id));
