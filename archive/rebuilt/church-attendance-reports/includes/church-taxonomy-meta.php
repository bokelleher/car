...
// Register and render custom admin columns for Church taxonomy

add_filter('manage_edit-church_columns', 'car_church_custom_columns');
add_filter('manage_church_custom_column', 'car_render_church_custom_columns', 10, 3);

/**
 * Modify the column headers
 */
function car_church_custom_columns($columns) {
    // Remove description column
    unset($columns['description']);

    // Add custom fields
    $columns['pastor_name'] = 'Pastor';
    $columns['city'] = 'City';
 ...
 */
function car_render_church_custom_columns($content, $column_name, $term_id) {
    global $wpdb;
    $table = $wpdb->prefix . 'car_churches';
    ...
    switch ($column_name) {
        ...
        case 'city':
            // Try to extract city from address (simple split)
            $parts = explode(',', $church->address);
            return isset($parts[1]) ? esc_html(trim($parts[1])) : '';
        case 'website':
            return $church->website ? '<a href="' . esc_url($church->website) . '" target="_blank">' . esc_html($church->website) . '</a>' : '';
        default:
            return $content;
    }
}
