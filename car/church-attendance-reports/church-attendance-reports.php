...
// Define the SMS API key constant.  To avoid shipping a secret key in the
// plugin, this constant is left blank. Administrators should enter
// their API key via the Attendance Reports settings page. The key is
// retrieved from the `car_clearstream_api_key` option defined in
// `clearstream-reminders.php`. If `car_clearstream_api_key` is empty,
// reminders will be disabled.
if (!defined('CAR_CLEARSTREAM_API_KEY')) {
    define('CAR_CLEARSTREAM_API_KEY', '');
}
...
// Include My Account menu injection and shortcode
// require_once plugin_dir_path(__FILE__) . 'includes/menu-my-account.php'; // Disabled: menu link now managed via WordPress menus
require_once plugin_dir_path(__FILE__) . 'includes/shortcode-my-account.php';
require_once plugin_dir_path(__FILE__) . 'includes/enqueue-my-account.php';
...
// Include our ClearStream reminder scheduler and sender. This file adds a
// weekly cron event that notifies church reporters and admins via SMS. It
// must be loaded after user meta and other dependencies are defined.
require_once plugin_dir_path(__FILE__) . 'includes/clearstream-reminders.php';
    ...
    <div class="wrap">
        ...
        if (empty($google_api_key)) {
            ...
            <div class="notice notice-error">
                ...
                <p>Please add your API key in <a href="<?php echo admin_url('edit.php?post_type=attendance_report&page=car_attendance_settings'); ?>">Attendance Reports → Settings</a></p>
            </div>
            <?php
            return;
        }
...
/**
 * Ensure the My Account page exists on plugin activation.
 *
 * When the plugin is activated for the first time, create a page
 * at the path `/my-account/` that contains the [car_my_account] shortcode.
 * If a page with that path already exists, no action is taken.
 */
function car_ensure_my_account_page() {
    // Only run on activation in the admin context
    if (get_option('car_my_account_page_created')) {
        return;
    }

    // Check if a page with the slug 'my-account' already exists
    ...
    if (!$existing_page) {
        ...
        wp_insert_post([
            ...
            'post_content' => '[car_my_account]',
            'post_status'  => 'publish',
            'post_type'    => 'page',
        ]);
    }

    // Flag that we've created the page so we don't duplicate it
    update_option('car_my_account_page_created', 1);
}

// Register activation hook to create the My Account page
register_activation_hook(__FILE__, 'car_ensure_my_account_page');