...
class CAR_Church_Finder {
    ...
    public function enqueue_map_assets() {
        // Only load assets on pages containing the church_finder_map shortcode. Use
        // the global $post instead of get_post() because get_post() may return
        // null during the wp_enqueue_scripts hook.
        if (is_admin()) {
            return;
        }
        global $post;
        if ($post && isset($post->post_content) && has_shortcode($post->post_content, 'church_finder_map')) {
            ...
            // Register Google Maps script with the API key and callback. We'll enqueue it
            // immediately; the dependency ensures car-church-finder loads first.
            wp_register_script(
                'google-maps',
                // Load the Maps JS API with the callback. We omit the loading=async param to
                // improve compatibility with browsers like Safari and Brave.
                'https://maps.googleapis.com/maps/api/js?key=' . $this->google_maps_api_key . '&callback=initChurchMap',
                ['car-church-finder'],
                null,
                false
            );
...
/*
 * The Google Maps API settings are now registered and rendered in
 * includes/admin-settings.php. The legacy callback and settings field
 * registration in this file have been removed to avoid function name
 * collisions (which would cause a fatal error on activation). If you need
 * to modify the settings UI, please update includes/admin-settings.php.
 */